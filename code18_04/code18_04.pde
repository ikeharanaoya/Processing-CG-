...
void setup() {
  size(640, 360);
  // 接続したWebカメラの「名前」、「解像度サイズ」「fps」のリストを配列camerasに文字列をとして格納する
  String[] cameras = Capture.list();
  // 配列camerasの中の文字列をコンソールに表示する
  printArray(cameras);
  // Captureオブジェクトを作り、リスト１番目のフォーマットでキャプチャデータをvideoに読み込む
...
void draw() {
  // 画面にスナップショットをpixelArray(ピクセルの配列)に入れる
  video.loadPixels();
  ...
  // 画面内の全てのx方向とy方向のピクセルを順番にチェックしていく
